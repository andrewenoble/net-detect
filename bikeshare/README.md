...
python filter.py
```
Run the python scripts in ```visualization``` and ```analysis``` to re-generate the UK measles outbreak animation [here](http://two.ucdavis.edu/~andrewnoble/research.html) and the plots [here](http://two.ucdavis.edu/~andrewnoble/bikeshare.html).  Running the scripts will overwrite the existing animation and plots.
